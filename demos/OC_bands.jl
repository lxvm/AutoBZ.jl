...
@info "Integrated :full  at $(length(Ωs)) points in $t_orbit s"
...
plot!(plt, Ωs, map(real∘first, σ_orbit); label="full")
...
err_plt = plot(; ylim=(1e-16, 1), yscale=:log10, xguide="Ω (eV)", yguide="|σ_orbit- σ_sum|", title="SrVO3 OC, $npt kpts/dim, η=$η eV, β=$(round(β,digits=2)) ev⁻¹")
