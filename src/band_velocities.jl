...
"""
    BandEnergyVelocity(H::FourierSeries{N}, V::ManyFourierSeries{N}, kind::Symbol) where N
...
they want other units, which can usually be done as a post-processing step. The
keyword `kind` allows the following options:
...
BandEnergyVelocity(H::FourierSeries{N}, kind::Symbol=:full) where N = BandEnergyVelocity(H, ManyFourierSeries((), period(H)), kind)
...
function contract(f::BandEnergyVelocity{N}, x::Number) where N
    ...
    BandEnergyVelocity(contract(f.H, x), ManyFourierSeries((contract(v, x), contract(f.V, x).fs...), pop(period(f))), f.kind)
end
value(f::BandEnergyVelocity{0}) = (value(f.H), band_velocities(f.kind, value(f.V))...)
...
post-processing step. The keyword `kind` is determines the band velocity component.
"""
...
function BandEnergyBerryVelocity(H::FourierSeries{N}, A::ManyFourierSeries{N,<:NTuple{N}}, kind::Symbol=:full) where N
    @assert period(H) == period(A)
    BandEnergyBerryVelocity(BandEnergyVelocity(H, kind), A)
end
...
function value(f::BandEnergyBerryVelocity{0})
    H, vs... = value(f.HV)
    As = value(f.A)
    (H, band_velocities(f.HV.kind, ntuple(n -> vs[n] - im*(H*As[n] - As[n]*H), Val{length(As)}()))...)
end
