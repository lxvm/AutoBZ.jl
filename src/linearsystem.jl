...
function init(prob::LinearSystemProblem, alg::LinearSystemAlgorithm;
    alias=false, kws...)

    (; A, Pl, Pr, kwargs) = prob
    alias && error("not implemented")
...
function init_linear_cacheval(A, Pl, Pr, alg::LUFactorization)
    return ismutable(A) ? LUWs(A) : nothing
end
function do_linear_solve!(A, Pl, Pr, alg::LUFactorization, ws; kws...)
    value = ismutable(A) ? LU(LAPACK.getrf!(ws, A)...) : lu(A)
...
"""
struct Inverse <: LinearSystemAlgorithm end

function init_linear_cacheval(A, Pl, Pr, alg::Inverse)
    # TODO allocate workspace for LAPACK.getri!
    return ismutable(A) ? init_linear_cacheval(A, Pl, Pr, LUFactorization()) : nothing
end
function do_linear_solve!(A, Pl, Pr, alg::Inverse, cacheval; kws...)
    invA = if ismutable(A)
    ...
    value = InverseMatrix(invA)
    retcode = Success
    stats = (;)
    return LinearSystemSolution(value, retcode, stats)
end

struct InverseMatrix{A}
    A::A
end
Base.inv(A::InverseMatrix) = A.A
Base.:\(A::InverseMatrix, b) = A.A * b
LinearAlgebra.ldiv!(x, A::InverseMatrix, b) = mul!(x, A.A, b)
