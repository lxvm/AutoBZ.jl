...
for d in 1:3
    ...
    for hv in [
        ...
        for alg in (PTR(; npt=50), IAI(), AutoPTR(a=η))
            solver = AuxTransportDistributionSolver(Σ, hv, bz, alg; ω₁, ω₂, μ, abstol, reltol)
            sol1 = solve!(solver)
            AutoBZ.update_auxtd!(solver; ω₁=ω₂, ω₂=ω₁, μ)
            sol2 = solve!(solver)
            @test sol1.value.val ≈ transpose(sol2.value.val) atol=abstol rtol=reltol
        end
    end
end
