...
export DOSEvaluator, TransportEvaluator, KineticEvaluator
...
function DOSEvaluator(H::TH, Σ, l; order=4, atol=nothing, rtol=nothing, norm=norm) where TH
    ...
    DOSEvaluator(H, Σ, l, order, atol_, rtol_, norm, segbufs)
end
(D::DOSEvaluator)(ω) = first(iterated_integration(DOSIntegrand(D.H, ω, D.Σ), D.l; order=D.order, atol=D.atol, rtol=D.rtol, norm=D.norm, segbufs=D.segbufs))
