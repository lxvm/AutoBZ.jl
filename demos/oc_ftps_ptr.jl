...
# TODO: define limits of integration dynamically as a function of Ω
oc_solver = IntegralSolver(oc_integrand, lb(Σ), ub(Σ)-maximum(Ωs), falg, (transport_solver, β); abstol=atol, reltol=rtol)

# run calculation
nthreads = kalg isa AutoPTR ? 1 : Threads.nthreads() # kpt parallelization (by default is preferred for large k-grids)
batchsolve("oc_ftps_ptr.h5", oc_solver, Ωs, RT; nthreads=nthreads)
