function adaptive_integration_sweep(T, f, ϵ, ωs, η::Number; μ=12.3958, callback=contract, kwargs...)
    ...
    for (i, ω) in enumerate(ωs)
        g = f(ϵ, ω, η, μ)
        t = time()
        @inbounds ints[i], errs[i] = iterated_integration(g, TetrahedralLimits(); callback=callback, kwargs...)
