...
for d in 1:3
    ...
    for h in [
        ...
        for alg in (PTR(; npt=50), IAI(), AutoPTR(a=η))
            solver = KineticCoefficientSolver(Σ, falg, h, bz, alg; β, μ, Ω, n, abstol, reltol)
            sol1 = solve!(solver)
            AutoBZ.update_kc!(solver; β, μ, Ω, n)
            sol2 = solve!(solver)
            # display(sol2.value)
            # @test sol1.value-0.5 ≈ -(sol2.value-0.5) atol=abstol rtol=reltol
            solver = KineticCoefficientSolver(h, bz, alg, Σ, falg; β, μ, Ω, n, abstol, reltol)
            sol3 = solve!(solver)
            AutoBZ.update_kc!(solver; β, μ, Ω, n)
            sol4 = solve!(solver)
